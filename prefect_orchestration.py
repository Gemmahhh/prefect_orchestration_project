...
from prefect_dbt.cloud import DbtCloudCredentials, run_dbt_cloud_job
...
@task
def load_to_snowflake(s3_path: str, snowflake_block_name: str, table_name: str):
    """
...
def trigger_dbt_cloud_job(dbt_block_name: str, job_id: int):
    ...
    job_run = run_dbt_cloud_job(dbt_cloud_credentials=dbt_credentials, job_id=job_id)
    print(f"Triggered dbt Cloud job. Run ID: {job_run}")
...
def data_pipeline_flow():
    ...
    load_to_snowflake(s3_path, snowflake_block_name, table_name)
